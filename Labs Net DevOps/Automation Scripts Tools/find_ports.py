...
def printhelp():
    ...
    printusertext('access policy currently configured. This script does not change configuration;')
    printusertext('its counterpart script update_ports.py does change the access policy.')
    printusertext('')
    printusertext('Usage:')
    printusertext('python find_ports.py -k <api_key> -o <org_id> -s <search_parameter> [-t <time>]')
...
def main(argv):
    ...
    API_KEY = ORG_ID = ARG_SEARCH = ARG_TIME = 'null'

    # Get command line arguments
    try:
        opts, args = getopt.getopt(argv, 'hk:o:s:t:')
    ...
    if API_KEY == 'null' or ORG_ID == 'null' or ARG_SEARCH == 'null':
        printhelp()
        sys.exit(2)
    ...
    if search_file is not None:
        ...
        for switch in switches:
            ...
            if len(matched_ports) > 0:
                print('Found %d matched ports on switch %s' % (len(matched_ports), switch['serial']))
